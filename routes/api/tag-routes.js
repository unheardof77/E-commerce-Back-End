...
router.put('/:id', async (req, res) => {
  ...
  try{
    ...
    if(updatedTag == 0) return res.status(404).json({message:"No tag found with that id."});
...
router.delete('/:id', async (req, res) => {
  ...
  try{
    ...
    if(deletedTag == 0) return res.status(404).json({message:"No tag found with that id."});
